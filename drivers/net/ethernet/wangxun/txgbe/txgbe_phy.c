...
/*
MTD_SEM txgbe_sem_create(MTD_SEM_BEGIN_STATE state)
{
					  ...
					  u16 *avago_data)
{
	...
	return txgbe_read_i2c_2bytes(hw, byte_offset,
...
/**
*  txgbe_read_i2c_2bytes_int - Reads 16 bit word over I2C
*  @hw: pointer to hardware structure
*  @byte_offset: byte offset to read
*  @data: value read
*  @lock: true if to take and release semaphore
...
*  a specified device address.
**/
STATIC s32 txgbe_read_i2c_2bytes_int(struct txgbe_hw *hw, u16 byte_offset,
					  u8 dev_addr, u16 *data, bool lock)
{
	...
	/* write reg_offset */
	wr32(hw, TXGBE_I2C_DATA_CMD, byte_offset >> 8 | TXGBE_I2C_DATA_CMD_STOP);
	wr32(hw, TXGBE_I2C_DATA_CMD, (u8)byte_offset);
...
/**
 *	txgbe_read_i2c_2bytes - Reads 16 bit word over I2C
 *	@hw: pointer to hardware structure
 *	@byte_offset: byte offset to read
 *	@data: value read
 *
 ...
 *	a specified device address.
 **/
s32 txgbe_read_i2c_2bytes(struct txgbe_hw *hw, u16 byte_offset,
				u8 dev_addr, u16 *data)
{
	txgbe_switch_i2c_slave_addr(hw, dev_addr);

	return txgbe_read_i2c_2bytes_int(hw, byte_offset, dev_addr,
						   data, true);
}
...
{
	return mtdUnloadDriver(&hw->phy_dev);
}
