...
void txgbe_dcb_get_rtrup2tc(struct txgbe_hw *hw, u8 *map)
{
	...
	for (i = 0; i < TXGBE_DCB_MAX_USER_PRIORITY; i++)
		map[i] = TXGBE_RDB_UP2TC_UP_MASK &
			(reg >> (i * TXGBE_RDB_UP2TC_UP_SHIFT));
	return;
}
...
s32 txgbe_get_bus_info(struct txgbe_hw *hw)
{
	...
	link_status = TXGBE_READ_PCIE_WORD(hw, TXGBE_PCI_LINK_STATUS);
...
s32 txgbe_disable_pcie_master(struct txgbe_hw *hw)
{
	...
	if(!vf_bme_clear) {
		...
		ERROR_REPORT1(TXGBE_ERROR_POLLING, "PCI VID is 0x%x\n", vid);

		pci_read_config_word(adapter->pdev, PCI_COMMAND, &cmd);
		ERROR_REPORT1(TXGBE_ERROR_POLLING, "PCI COMMAND value is 0x%x.\n", cmd);

		reg32 = rd32(hw, 0x10000);
		ERROR_REPORT1(TXGBE_ERROR_POLLING, "read 0x10000 value is 0x%08x\n", reg32);
	}
...
u8 txgbe_flash_erase_chip(struct txgbe_hw *hw)
{
	u8 status = fmgr_cmd_op(hw, SPI_CMD_ERASE_CHIP, 0);
	return status;
}

u8 txgbe_flash_erase_sector(struct txgbe_hw *hw, u32 sec_addr)
{
...
u8 txgbe_flash_write_dword(struct txgbe_hw *hw, u32 addr, u32 dword)
{
	u8 status = 0;
	...
	if (dword != txgbe_flash_read_dword(hw, addr)) {
		return 1;
	}
	return 0;
}
								...
								const u8 *data, u32 size)
{
	...
	mac_addr0_dword0_t = txgbe_flash_read_dword(hw, MAC_ADDR0_WORD0_OFFSET_1G);
	mac_addr0_dword1_t = txgbe_flash_read_dword(hw, MAC_ADDR0_WORD1_OFFSET_1G) & 0xffff;
	mac_addr1_dword0_t = txgbe_flash_read_dword(hw, MAC_ADDR1_WORD0_OFFSET_1G);
	mac_addr1_dword1_t = txgbe_flash_read_dword(hw, MAC_ADDR1_WORD1_OFFSET_1G) & 0xffff;

	serial_num_dword0_t = txgbe_flash_read_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G);
	serial_num_dword1_t = txgbe_flash_read_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G+4);
	serial_num_dword2_t = txgbe_flash_read_dword(hw, PRODUCT_SERIAL_NUM_OFFSET_1G+8);
	...
	for (i=0; i<size/4; i++) {
		...
		if (read_data != 0xffffffff && !skip) {
			...
			if (status) {
				...
				read_data = txgbe_flash_read_dword(hw, i*4);
...
s32 txgbe_init_ops(struct txgbe_hw *hw)
{
	...
	mac->ops.get_rtrup2tc = txgbe_dcb_get_rtrup2tc;
...
s32 txgbe_set_link_to_kr(struct txgbe_hw *hw, bool autoneg)
{
	...
	if (1) {
		...
		/*
		txgbe_wr32_epcs(hw, TXGBE_SR_AN_MMD_ADV_REG2, 0x80);
...
void txgbe_disable_fdir(struct txgbe_hw *hw)
{
	...
	if (!hw->Fdir_enabled) {
		wr32(hw, TXGBE_RDB_FDIR_CTL, 0x4AF07F18);
		TXGBE_WRITE_FLUSH(hw);
	}
	val = rd32(hw, TXGBE_RDB_FDIR_CTL);
}
#endif
...
#if 0
	/* need fix */
	switch (input_mask->formatted.tunnel_type & 0xFFFFFFFF) {
	case 0x0:
		/* Mask turnnel type, fall through */
		fdirm |= TXGBE_RDB_FDIR_OTHER_MSK_TUN |
			TXGBE_RDB_FDIR_OTHER_MSK_TUN_OUTIP |
			TXGBE_RDB_FDIR_OTHER_MSK_TUN_TYPE;
	case 0xFFFFFFFF:
		break;
	default:
...
bool txgbe_is_lldp(struct txgbe_hw *hw)
{
	struct txgbe_adapter *adapter = hw->back;
	struct pci_dev *pdev = adapter->pdev;
	u32 tmp = 0;
	...
	for (; i < 0x1000 / sizeof(u32); i++) {
        tmp = txgbe_flash_read_dword(hw, TXGBE_LLDP_REG + i * 4);
        if (tmp != 0xffffffff) {
            lldp_flash_data = tmp;
        } else {
            break;
