 ...
 *
 * based on kcompat.c, Copyright(c) 1999 - 2017 Intel Corporation.
 * Contact Information:
		      ...
		      u16 num_tx_queues)
{
	...
	if (netdev_get_num_tc(dev)) {
		...
		if (skb->priority == TC_PRIO_CONTROL) {
			...
			qoffset &= TXGBE_TX_FLAGS_VLAN_PRIO_MASK;
			qoffset >>= 13;
		}
...
{
	struct adapter_struct *kc_adapter = netdev_priv(dev);
	if (kc_adapter->flags & TXGBE_FLAG_DCB_ENABLED)
...
int _kc_netdev_set_num_tc(struct net_device *dev, u8 num_tc)
{
	...
	if (num_tc > TXGBE_DCB_MAX_TRAFFIC_CLASS)
		return -EINVAL;
...
static inline int kc_get_xps_queue(struct net_device *dev, struct sk_buff *skb)
{
	...
	if (kc_adapter->flags & TXGBE_FLAG_FDIR_HASH_CAPABLE) {
